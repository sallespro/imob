...
export default function FilterPanel({ filters, onChange, onExtract, isExtracting }) {
  ...
  const displayedTypes = showAllTypes ? PROPERTY_TYPES : PROPERTY_TYPES.slice(0, 4);
    ...
    <aside className="filter-panel">
      ...
      <div className="filter-panel-body">
        ...
        <Section title="Tipo de Imóvel">
          <div className="checkbox-list">
            {displayedTypes.map(t => (
              <label key={t} className="checkbox-item">
                <input
                  type="checkbox"
                  checked={filters.tipoImovel.includes(t)}
                  onChange={e => {
                    const next = e.target.checked
                      ? [...filters.tipoImovel, t]
                      : filters.tipoImovel.filter(x => x !== t);
                    set('tipoImovel', next);
                  }}
                />
                <span>{t}</span>
          ...
          <button className="show-more-btn" onClick={() => setShowAllTypes(s => !s)}>
            {showAllTypes ? 'Ver menos' : `Ver mais (${PROPERTY_TYPES.length - 4})`}
          </button>
        </Section>
        ...
        <Section title="Lançamentos" defaultOpen={false}>
          <ToggleGroup
            options={[
              { value: 'Sim', label: 'Sim' },
              { value: 'Nao', label: 'Não' },
        ...
        <Section title="Destaques" defaultOpen={false}>
          <div className="checkbox-list">
            <label className="checkbox-item">
              <input
                type="checkbox"
                checked={filters.exclusivo}
                onChange={e => set('exclusivo', e.target.checked)}
              />
              <span>Exclusivo</span>
            </label>
            <label className="checkbox-item">
            ...
            <label className="checkbox-item">
              <input
                type="checkbox"
                checked={filters.tags?.includes('avalia-imovel')}
                onChange={e => {
                  const next = e.target.checked
                    ? [...(filters.tags || []), 'avalia-imovel']
                    : (filters.tags || []).filter(t => t !== 'avalia-imovel');
                  set('tags', next);
                }}
              />
              <span>Avalia imóvel no negócio</span>
            </label>
            <label className="checkbox-item">
              <input
                type="checkbox"
        ...
        <Section title="Comodidades" defaultOpen={false}>
          <div className="checkbox-list">
            {displayedAmenities.map(a => (
              <label key={a.value} className="checkbox-item">
                <input
                  type="checkbox"
                  checked={filters.comodidades.includes(a.value)}
                  onChange={e => {
                    const next = e.target.checked
                      ? [...filters.comodidades, a.value]
                      : filters.comodidades.filter(x => x !== a.value);
                    set('comodidades', next);
                  }}
                />
                <span>{a.label}</span>
              </label>
          ...
          <button className="show-more-btn" onClick={() => setShowAllAmenities(s => !s)}>
            {showAllAmenities ? 'Ver menos' : `Ver mais (${AMENITIES.length - 6})`}
          </button>
        </Section>

        <Section title="Paginação da extração" defaultOpen={false}>
          <div className="input-group">
